...
def import_phishtank(entries: dict[str, dict[str, Any]]) -> None:
    ...
    if not key:
        print("PhishTank skipped: PHISHTANK_APP_KEY secret is not set")
        return
    url = f"https://data.phishtank.com/data/{urllib.parse.quote(key)}/online-valid.json"
    try:
        data = json.loads(fetch_text(url))
    except Exception as exc:
        print(f"PhishTank skipped: {exc}")
        return
    ...
    for item in data:
        ...
        upsert(entries, {
            ...
            "source": "PhishTank verified" + (f" ({item.get('phish_detail_url')})" if item.get("phish_detail_url") else ""),
